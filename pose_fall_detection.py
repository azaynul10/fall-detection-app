...
class PoseFallDetector:
    ...
    def process_frame(self, frame):
        ...
        Returns:
            tuple: (annotated_frame, fallen)
                annotated_frame (numpy.ndarray): The processed frame with annotations.
                fallen (bool): True if a fall is detected, otherwise False.
        """
        try:
            if not self.paused:
                self.frame_count += 1
                ...
                if results.pose_landmarks:
                    ...
                    if all(landmarks[landmark].visibility > 0.5 for landmark in required_landmarks):
                        ...
                        fallen = (
                            (vertical_displacement > self.fall_threshold and avg_hip_y > self.ground_threshold)
                            or nose_above_hips
                            or (angle > 60)  # Consider fallen if body is tilted more than 60 degrees
                        )
